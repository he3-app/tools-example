    ...
    <h-horizontal-layout class="layout">
        <template #left>
            <div class="left">
                ...
                <h-code-editor v-model="input" :save-options="{autoSave: true, key: 'mermaid-editor-h-code-editor'}"
                               :lang="'MARKDOWN'" class="left-editor"/>
        ...
        <template #right>
            <div class="right">
                ...
                <div class="mermaid right-container" ref="container"></div>
            </div>
        </template>
    </h-horizontal-layout>
</template>
...
script.type = 'module';
script.src = 'https://cdn.jsdelivr.net/npm/mermaid/dist/mermaid.js';
...
async function draw() {
    ...
    mermaid.mermaidAPI
        .render('mermaidGraph', graphDefinition)
        .then((graph) => {
            download.value.svgDisabled = true;
            element.innerHTML = graph.svg;
            err.value = '';
