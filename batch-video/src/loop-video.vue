  ...
  <VideoComponents @upload='upload' @remove='remove' :loading='loading'>
    <h-card-box text='Setting'>
      <div class='itemOption'>
        <div class='title'>{{ t('loopVideo.count') }}</div>
        <a-radio-group v-model:value='loop'>
          ...
          <a-radio-button value='6'>6x</a-radio-button>
        </a-radio-group>
      </div>
    </h-card-box>
    <div class='options'>
...
const convertFunction = async () => {
  ...
  await ffmpeg.run('-stream_loop', `${Number(loop.value) - 1}`, '-i', name, '-c', 'copy', 'output.mp4');
  // Read the result
